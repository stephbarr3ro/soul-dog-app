...
import { ChevronLeft, ChevronRight, ShoppingBag, BookOpen, X, Check } from 'lucide-react';
...
export const Customize = () => {
  ...
  const { edition, setEdition, children, dogs, storyTitle, coverColor, dedication, giftWrapping, reset } = useCustomizationStore();
  ...
  useEffect(() => {
    ...
    if (editionParam === 'classic' || editionParam === 'true-likeness') {
      ...
      setCurrentStep(1); // Skip edition choice if coming from homepage CTA
    }
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      if (!product) {
        throw new Error('Product not found in Shopify. Please check your configuration.');
      }

      // Prepare line item properties from customization state
      ...
      children.forEach((child, index) => {
        customProperties[`Child ${index + 1} Name`] = child.name;
        customProperties[`Child ${index + 1} Gender`] = child.gender;
        customProperties[`Child ${index + 1} Age`] = child.ageRange;
        if (child.photo) customProperties[`Child ${index + 1} Photo`] = 'Uploaded';
      });

      // Add dog details
      dogs.forEach((dog, index) => {
        customProperties[`Dog ${index + 1} Name`] = dog.name;
        customProperties[`Dog ${index + 1} Breed`] = dog.breed;
        if (dog.photo) customProperties[`Dog ${index + 1} Photo`] = 'Uploaded';
      });
      ...
      alert('We encountered an issue connecting to Shopify. Please ensure your environment variables are configured.');
    } finally {
    ...
    <div className="min-h-screen bg-white flex flex-col md:flex-row">
      ...
      <div className="w-full md:w-1/2 p-6 md:p-10 lg:p-16 overflow-y-auto max-h-screen scrollbar-hide">
        ...
        <div className="max-w-xl mx-auto pb-24">
          ...
          {/* Navigation Buttons */}
          <div className="fixed bottom-0 left-0 w-full md:w-1/2 bg-white/80 backdrop-blur-2xl border-t border-gray-100 p-6 flex gap-4 z-40">
            <button
              onClick={handleBack}
              className="px-8 py-4 rounded-full border border-gray-200 font-bold text-navy/40 hover:border-navy hover:text-navy transition-all flex items-center gap-2 text-xs uppercase tracking-widest"
            >
      ...
      <div className="hidden md:flex w-1/2 bg-navy items-center justify-center p-12 lg:p-24 relative overflow-hidden">
        <div className="absolute inset-0 opacity-20">
          <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-gold/10 to-transparent" />
        </div>
        <LivePreview className="w-full h-full relative z-10" />
      </div>

      {/* Mobile Floating Preview Trigger */}
      <button
        onClick={() => setIsPreviewOpen(true)}
        className="md:hidden fixed bottom-28 right-6 w-14 h-14 bg-gold text-cream rounded-full shadow-2xl z-40 flex items-center justify-center"
      >
      ...
      <AnimatePresence>
        {isPreviewOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="md:hidden fixed inset-0 bg-navy z-[100] flex flex-col p-8 pt-24"
          >
            <button 
              onClick={() => setIsPreviewOpen(false)} 
              className="absolute top-8 right-8 text-cream p-3 hover:bg-white/10 rounded-full transition-colors"
            >
              <X className="w-8 h-8" />
            </button>
            ...
            <p className="text-center text-cream/30 text-[10px] font-bold uppercase tracking-[0.3em] mt-12">
              Soul Dog Stories™
            </p>
          </motion.div>
        )}
