...
import { useCustomizationStore, Child } from '@/src/store/useCustomizationStore';
...
const HAIR_COLORS = [
  ...
  { name: "Medium Brown", hex: "#785032" },
  { name: "Dark Brown", hex: "#4B321E" },
  { name: "Black", hex: "#1A1A1A" },
  { name: "Red", hex: "#B22222" },
  { name: "Auburn", hex: "#8B4513" },
  { name: "White", hex: "#F5F5F5" },
  { name: "Grey", hex: "#808080" },
  { name: "Salt & Pepper", hex: "#404040" },
          ...
          >
            ...
            <div className="grid gap-8">
                      ...
                      <button
                        ...
                        onClick={() => {
                          updateChild(child.id, { 
                            gender: g as 'Girl' | 'Boy',
                            hairStyle: HAIR_STYLES[g as 'Girl' | 'Boy'][0]
                          });
                        }}
                        className={cn(
                          "flex-1 py-3 rounded-xl text-[11px] font-bold uppercase tracking-widest transition-all",
                          child.gender === g ? "bg-white text-navy shadow-lg" : "text-navy/30 hover:text-navy/60"
                        )}
                      >
                        {g}
                      </button>
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    <select
                      ...
                      onChange={(e) => updateChild(child.id, { ageRange: e.target.value as any })}
                    ...
                    >
                      {['Baby', 'Toddler', '3-5', '6-8', '9-12'].map((a) => (
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-3">
                  {SKIN_TONES.map((tone, i) => (
                    <button
                      key={i}
                      onClick={() => updateChild(child.id, { skinTone: `tone-${i + 1}` })}
                      className={cn(
                        "w-10 h-10 rounded-full border-2 transition-all duration-500 hover:scale-110",
                        child.skinTone === `tone-${i + 1}` ? "border-gold scale-110 shadow-xl" : "border-white shadow-sm"
                      )}
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  <select
                    value={child.hairStyle}
                    onChange={(e) => updateChild(child.id, { hairStyle: e.target.value })}
                    className="w-full p-4 rounded-2xl border border-gray-100 bg-gray-50/30 text-navy font-medium outline-none focus:border-gold appearance-none"
                  >
                    {HAIR_STYLES[child.gender].map((style) => (
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-3">
                  {HAIR_COLORS.map((color) => (
                    <button
                      key={color.name}
                      onClick={() => updateChild(child.id, { hairColor: color.name })}
                      className={cn(
                        "w-10 h-10 rounded-full border-2 transition-all duration-500 hover:scale-110",
                        child.hairColor === color.name ? "border-gold scale-110 shadow-xl" : "border-white shadow-sm"
                      )}
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  <select
                    ...
                    onChange={(e) => updateChild(child.id, { eyeColor: e.target.value })}
                    className="w-full p-4 rounded-2xl border border-gray-100 bg-gray-50/30 text-navy font-medium outline-none focus:border-gold appearance-none"
                  >
                    {EYE_COLORS.map((eye) => (
              ...
              <div className="flex flex-wrap gap-6 pt-2">
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input
                    type="checkbox"
                    checked={child.glasses}
                    onChange={(e) => updateChild(child.id, { glasses: e.target.checked })}
                    className="hidden"
                  />
                  <div className={cn(
                    "w-7 h-7 rounded-xl border-2 flex items-center justify-center transition-all duration-500",
                    child.glasses ? "bg-gold border-gold text-white shadow-lg" : "border-gray-100 group-hover:border-gold/30"
                  )}>
                    {child.glasses && <Check className="w-4 h-4" />}
                  </div>
                  <span className="text-[11px] font-bold text-navy uppercase tracking-widest">Glasses</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input
                    type="checkbox"
                    checked={child.freckles}
                    onChange={(e) => updateChild(child.id, { freckles: e.target.checked })}
                    className="hidden"
                  />
                  <div className={cn(
                    "w-7 h-7 rounded-xl border-2 flex items-center justify-center transition-all duration-500",
                    child.freckles ? "bg-gold border-gold text-white shadow-lg" : "border-gray-100 group-hover:border-gold/30"
                  )}>
                    {child.freckles && <Check className="w-4 h-4" />}
                  </div>
                  <span className="text-[11px] font-bold text-navy uppercase tracking-widest">Freckles</span>
                </label>
              </div>
                ...
                <div className="pt-4 space-y-3">
                  ...
                  <div className="relative">
                    <input
                      ...
                      disabled={isUploading === child.id}
                    />
                    <div className={cn(
                      "border-2 border-dashed rounded-[2rem] p-8 text-center transition-all duration-700 group bg-gray-50/50",
                      isUploading === child.id ? "border-gold bg-gold/5" : "border-gray-100 hover:border-gold hover:bg-white hover:shadow-2xl"
                    )}>
                      {isUploading === child.id ? (
                        ...
                        <div className="relative w-24 h-24 mx-auto mb-4">
                          <img src={child.photo} alt="Preview" className="w-full h-full object-cover rounded-2xl shadow-xl" />
                      ...
                      <p className="text-[11px] font-bold text-navy/40 uppercase tracking-widest group-hover:text-navy transition-colors">
                        {isUploading === child.id ? "Uploading..." : child.photo ? "Change Reference Photo" : "Upload Reference Photo"}
                      </p>
        ...
        >
          ...
          <div className="relative z-10 space-y-6">
            ...
            <p className="text-cream/60 text-sm leading-relaxed max-w-md">
              Upgrade to the True Likeness Edition. Our artists will hand-draw your children from a photo for an exact representation.
            </p>
            <button
